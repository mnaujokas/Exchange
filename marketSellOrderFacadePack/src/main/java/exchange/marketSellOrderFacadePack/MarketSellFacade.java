package exchange.marketSellOrderFacadePack;
...
public class MarketSellFacade implements OrderSellable{
    ...
    public int placeSellOrder(String amount, String currency, String forCurrency, String price, String userID) {
        ...
        double Price = exchangeAPI.getPrice();
    ...
    public List<String> getOrders() {
        try {List<SellOrder> data = sellOrderRepoAPI.GetOrders();
            ...
            for(int i=0; i<data.size(); i++) {
                String line = "ID: "+ data.get(i).getID()+" Valiuta: "+ data.get(i).getCurrency() + " Kiekis: " + data.get(i).getAmount() +" Vieneto kaina:"+ data.get(i).getPrice() + " Levrage: " + data.get(i).getLeverage();
                orders.add(line);
    ...
    public boolean createWithdrawalRepo(String address, double amount, String currency, int orderID) {
        Withdrawal withdrawal = withdrawalFactory.createWithdrawal(address, amount, currency, orderID);
        try {
